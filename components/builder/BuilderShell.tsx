...
export type PlanCode = keyof typeof PLAN_LABELS;
...
function getDefaultPermissions(planCode: PlanCode): BuilderPermissions {
...
function mergePermissions(
  ...
  return {
    ...getDefaultPermissions(planCode),
    ...permissions,
  };
}
...
function PaidSessionBanner({
  ...
  return (
    <div className="mt-5 grid gap-3 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm text-slate-700 sm:grid-cols-2 lg:grid-cols-4">
      {email && (
        <div>
      ...
      {paymentId && (
        <div>
          ...
          <p className="truncate">{paymentId}</p>
        </div>
      )}

      {sessionId && (
        <div>
          <p className="font-bold text-slate-950">Session</p>
          <p className="truncate">{sessionId}</p>
        </div>
      )}

      {typeof aiCreditsRemaining === "number" && (
        <div>
          <p className="font-bold text-slate-950">AI Credits</p>
          <p>{aiCreditsRemaining} remaining</p>
        </div>
      )}
    </div>
  );
...
export default function BuilderShell({
  ...
  const activePermissions = mergePermissions(planCode, permissions);

  const step = steps[currentStep - 1];
  const isLastStep = currentStep === steps.length;

  const isBasicPlan = planCode === "basic";
  const isProfessionalPlan = planCode === "professional";

  useEffect(() => {
    if (planCode !== "basic") return;

    if ((resumeData.templateId ?? "basic") !== "basic") {
      setResumeData((previousData) => ({
        ...previousData,
        templateId: "basic",
      }));
    }
  }, [planCode, resumeData.templateId, setResumeData]);
          ...
          <section className="rounded-2xl bg-white p-5 shadow-lg sm:p-8 lg:p-10">
            ...
            <div className="mx-auto max-w-3xl">
              ...
              <div className="mt-8">
                ...
                {currentStep === 2 && (
                  <ProfessionalSummary
                    canUseAi={activePermissions.aiSummary}
                  />
                )}

                {currentStep === 3 && (
                  <WorkExperience
  canUseAi={activePermissions.aiExperience}
...
/>
                ...
                {currentStep === 6 && <Review
                  planCode={planCode}
                  canDownloadPdf={activePermissions.pdfDownload}
  ...
  {activePermissions.templates && (
    <TemplateSelector canUsePremiumTemplates />
  )}
   {activePermissions.atsScore && <ResumeScore />}

  {isProfessionalPlan && !activePermissions.coverLetter && (
    <LockedFeaturePrompt
  ...
  {activePermissions.coverLetter && <CoverLetterGenerator />}
</aside>
        </div>
      </div>
